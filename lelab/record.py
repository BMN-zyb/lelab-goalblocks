...
from .goals import GoalImage, save_goal_assets
...
class RecordingRequest(BaseModel):
    ...
    goal_images: list[GoalImage] = Field(default_factory=list, max_length=4)
        ...
        def recording_worker():
            ...
            try:
                ...
                dataset = record_with_web_events(
                    record_config, recording_events, request.goal_images, request.single_task
                )
...
def record_with_web_events(
    cfg: RecordConfig,
    web_events: dict,
    goal_images: list[GoalImage] | None = None,
    ...
    try:
        ...
        try:
            ...
            dataset.finalize()
            # save_episode updates info.json, so custom metadata must be added
            # only after the dataset's final metadata update. This also runs
            # when recording exits through an exception, preserving Goal data
            # for partial datasets that the frontend can still upload.
            if goal_images:
                save_goal_assets(dataset.root, goal_images, goal_task or cfg.dataset.single_task)
        finally:
